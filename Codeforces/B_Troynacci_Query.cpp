...
int matrixExponentiation(int a, int b, int f1, int f2, int n) {
    // 0 is degenerate case
    if (n == 1) {
        return f1;
    } else if (n == 2) {
        return f2;
    ...
    n -= 2;
    vector<vector<int>> T = {{a, b}, {1, 0}};
    vector<vector<int>> initial = {{f2, 0}, {f1, 0}};
...
void solve() {
    ...
    for (int i = 1; i <= 10; i++) {
        int f = matrixExponentiation(a, b, f1, f2, i);
        cout << "f" << i << " = " << f << "\n";
    }
}
